...
class Settings(PydanticBaseSettings):
    ...
    # SIP Configuration
    sip_domain: str = Field(..., description="SIP domain or IP address")
    sip_user: str = Field(..., description="SIP username")
    sip_pass: str = Field(..., description="SIP password")
    ...
    sip_jitter_buffer_size: int = Field(default=80, ge=20, le=400, description="Jitter buffer size in ms")
    sip_srtp_enabled: bool = Field(default=False, description="Enable SRTP encryption")
    sip_srtp_secure_signaling: bool = Field(default=False, description="Enable secure SIP signaling (TLS)")
    sip_nat_type: Optional[str] = Field(default=None, description="NAT type: STUN, TURN, or ICE")
    sip_stun_server: Optional[str] = Field(default=None, description="STUN server address")
    sip_turn_server: Optional[str] = Field(default=None, description="TURN server address")
    ...
    sip_codecs: List[SIPCodec] = Field(default=[SIPCodec.PCMU, SIPCodec.PCMA], description="Preferred codecs")
    sip_registration_timeout: int = Field(default=30, ge=5, le=300, description="Registration timeout in seconds")
    sip_registration_retry_max: int = Field(default=5, ge=1, le=20, description="Max registration retries")
    sip_registration_retry_backoff: float = Field(default=2.0, ge=1.0, le=10.0, description="Registration retry backoff multiplier")
    
    # RTP Configuration
    rtp_port_range_start: int = Field(default=16000, ge=1024, le=65535, description="Start of RTP port range")
    rtp_port_range_end: int = Field(default=16100, ge=1024, le=65535, description="End of RTP port range")
    
    # Audio Configuration
    audio_sample_rate: int = Field(default=16000, description="Audio sample rate in Hz")
    audio_channels: int = Field(default=1, ge=1, le=2, description="Number of audio channels")
    audio_frame_duration: int = Field(default=20, ge=10, le=100, description="Audio frame duration in ms")
    audio_backpressure_threshold: int = Field(default=100, ge=10, le=1000, description="WebSocket backpressure threshold")
    
    # OpenAI Configuration
    openai_api_key: str = Field(..., description="OpenAI API key")
    agent_id: str = Field(..., description="OpenAI agent ID")
    openai_mode: OpenAIAPIMode = Field(default=OpenAIAPIMode.LEGACY, description="OpenAI API mode")
    openai_model: str = Field(default="gpt-realtime", description="OpenAI model name")
    openai_voice: OpenAIVoice = Field(default=OpenAIVoice.ALLOY, description="OpenAI voice")
    openai_temperature: float = Field(default=0.3, ge=0.0, le=2.0, description="OpenAI temperature")
    system_prompt: str = Field(default="You are a helpful voice assistant.", description="System prompt")
    openai_timeout: int = Field(default=30, ge=5, le=300, description="OpenAI API timeout in seconds")
    openai_retry_max: int = Field(default=3, ge=1, le=10, description="Max OpenAI API retries")
    ...
    def validate_voice_for_mode(cls, v, values):
        ...
        if values.get('openai_mode') == OpenAIAPIMode.REALTIME:
            ...
            supported_voices = {OpenAIVoice.ALLOY, OpenAIVoice.ECHO, OpenAIVoice.FABLE, 
                              ...
                              OpenAIVoice.CEDAR, OpenAIVoice.MARIN}
