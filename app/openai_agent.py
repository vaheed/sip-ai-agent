...
class OpenAIAgent:
    ...
    def _validate_configuration(self) -> None:
        ...
        if self.settings.openai_mode == OpenAIAPIMode.REALTIME:
            ...
            if self.settings.openai_voice not in [
                OpenAIVoice.ALLOY, OpenAIVoice.ECHO, OpenAIVoice.FABLE,
                OpenAIVoice.ONYX, OpenAIVoice.NOVA, OpenAIVoice.SHIMMER,
                OpenAIVoice.CEDAR, OpenAIVoice.MARIN
            ]:
                raise ValueError(f"Voice {self.settings.openai_voice} not supported in realtime mode")
            
            # Validate model for realtime mode
            if self.settings.openai_model not in [model.value for model in OpenAIRealtimeModel]:
                raise ValueError(f"Model {self.settings.openai_model} not supported in realtime mode")
    ...
    async def start(self, call) -> None:
        ...
        with with_correlation_id(self.correlation_id):
            try:
                ...
                logger.error("Failed to start OpenAI agent",
                           correlation_id=self.correlation_id,
                           error=str(e))
    ...
    async def _start_legacy_agent(self, call) -> None:
        ...
        logger.info("Starting legacy OpenAI agent",
                   correlation_id=self.correlation_id)
        ...
        try:
            async with websockets.connect(ws_url, extra_headers=headers) as self.ws:
                ...
                log_openai_event("legacy_session_started",
                               correlation_id=self.correlation_id,
                               config=config)
            ...
            logger.error("Legacy agent error",
                        correlation_id=self.correlation_id,
                        error=str(e))
            raise
        finally:
    ...
    async def _start_realtime_agent(self, call) -> None:
        ...
        logger.info("Starting realtime OpenAI agent",
                   correlation_id=self.correlation_id)
        ...
        try:
            async with websockets.connect(ws_url, extra_headers=headers) as self.ws:
                ...
                log_openai_event("realtime_session_started",
                               correlation_id=self.correlation_id,
                               model=self.settings.openai_model,
                               voice=self.settings.openai_voice.value)
            ...
            logger.error("Realtime agent error",
                        correlation_id=self.correlation_id,
                        error=str(e))
            raise
        finally:
    ...
    async def _send_audio_legacy(self, call) -> None:
        ...
        try:
            while self.is_active and call.audio_callback and call.audio_callback.is_active:
                ...
                if audio_chunk and self.ws and not self.ws.closed:
                    ...
                    if frame_count % 100 == 0:  # Log every 100 frames
                        logger.debug("Legacy audio progress",
                                   correlation_id=self.correlation_id,
                                   frames=frame_count,
            ...
            logger.error("Legacy audio sender error",
                        correlation_id=self.correlation_id,
                        error=str(e))
        finally:
            logger.info("Legacy audio sender completed",
                       correlation_id=self.correlation_id,
    ...
    async def _send_audio_realtime(self, call) -> None:
        ...
        logger.info("Starting realtime audio sender",
                   correlation_id=self.correlation_id)
        ...
        try:
            while self.is_active and call.audio_callback and call.audio_callback.is_active:
                ...
                if audio_chunk and self.ws and not self.ws.closed:
                    ...
                    if frame_count % 100 == 0:  # Log every 100 frames
                        logger.debug("Realtime audio progress",
                                   correlation_id=self.correlation_id,
                                   frames=frame_count,
                                   total_bytes=total_bytes)
            ...
            logger.error("Realtime audio sender error",
                        correlation_id=self.correlation_id,
                        error=str(e))
        finally:
            logger.info("Realtime audio sender completed",
                       correlation_id=self.correlation_id,
                       total_frames=frame_count,
                       total_bytes=total_bytes)
    ...
    async def _receive_audio_legacy(self, call) -> None:
        ...
        logger.info("Starting legacy audio receiver",
                   correlation_id=self.correlation_id)
            ...
            while self.is_active and self.ws and not self.ws.closed:
                ...
                if isinstance(response, bytes):
                    ...
                    if frame_count % 10 == 0:  # Log every 10 frames
                        logger.debug("Legacy audio playback progress",
                                   correlation_id=self.correlation_id,
                                   frames=frame_count,
                                   total_bytes=total_bytes)
                
        except Exception as e:
            logger.error("Legacy audio receiver error",
                        correlation_id=self.correlation_id,
                        error=str(e))
        finally:
            logger.info("Legacy audio receiver completed",
                       correlation_id=self.correlation_id,
                       total_frames=frame_count,
                       total_bytes=total_bytes)
    ...
    async def _receive_audio_realtime(self, call) -> None:
        ...
        logger.info("Starting realtime audio receiver",
                   correlation_id=self.correlation_id)
        ...
        try:
            while self.is_active and self.ws and not self.ws.closed:
                ...
                if msg_type == 'response.output_audio.delta' and 'delta' in message:
                    ...
                    try:
                        ...
                        if frame_count % 10 == 0:  # Log every 10 frames
                            logger.debug("Realtime audio playback progress",
                                       correlation_id=self.correlation_id,
                                       frames=frame_count,
                                       total_bytes=total_bytes)
                        
                    except Exception as decode_err:
                        logger.error("Error decoding audio delta",
                                   correlation_id=self.correlation_id,
                    ...
                    if transcript.strip():
                        logger.debug("Transcription update",
                                   correlation_id=self.correlation_id,
            ...
            logger.error("Realtime audio receiver error",
                        correlation_id=self.correlation_id,
                        error=str(e))
        finally:
            logger.info("Realtime audio receiver completed",
                       correlation_id=self.correlation_id,
                       total_frames=frame_count,
                       total_bytes=total_bytes)
    ...
    def stop(self) -> None:
        ...
        logger.info("OpenAI agent stopped",
                   correlation_id=self.correlation_id,
                   duration=duration)
