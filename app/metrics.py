...
sip_calls_total = Counter(
    "sip_calls_total", "Total number of SIP calls", ["direction", "result"]
)

sip_call_duration = Histogram(
    "sip_call_duration_seconds", "Duration of SIP calls", ["result"]
)
...
audio_queue_size = Gauge("audio_queue_size", "Current size of audio queue", ["call_id"])

audio_dropouts = Counter(
    "audio_dropouts_total", "Total number of audio dropouts", ["call_id", "reason"]
)
...
openai_websocket_connections = Gauge(
    "openai_websocket_connections", "Number of active OpenAI WebSocket connections"
)
...
call_setup_time = Histogram(
    "call_setup_time_seconds", "Time taken to set up a call", ["result"]
)

end_to_end_latency = Histogram(
    "end_to_end_latency_seconds", "End-to-end audio latency", ["call_id"]
)
...
class MetricsCollector:
    ...
    # SIP Metrics
    def update_sip_registration_status(
        self, domain: str, user: str, registered: bool
    ) -> None:
        """Update SIP registration status."""
        sip_registration_status.labels(domain=domain, user=user).set(
            1 if registered else 0
        )
        logger.info(
            "SIP registration status updated",
            domain=domain,
            user=user,
            registered=registered,
        )

    def record_sip_registration_attempt(
        self, domain: str, user: str, success: bool, duration: float
    ) -> None:
        ...
        logger.info(
            "SIP registration attempt recorded",
            domain=domain,
            user=user,
            success=success,
            duration=duration,
    ...
    def record_audio_frame(
        self, call_id: str, direction: str, processing_time: float
    ) -> None:
