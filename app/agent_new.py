...
from logging_config import setup_logging, get_logger, with_correlation_id, generate_correlation_id
...
class SIPAIAgent:
    ...
    async def initialize(self) -> None:
        ...
        with with_correlation_id(self.correlation_id):
            try:
                self.logger.info("Initializing SIP AI Agent",
                               version="2.1.0",
    ...
    def _validate_configuration(self) -> None:
        ...
        required_settings = [
            'sip_domain', 'sip_user', 'sip_pass', 
            'openai_api_key', 'agent_id'
        ]
        ...
        if self.settings.audio_sample_rate not in [8000, 16000, 22050, 44100, 48000]:
            raise ValueError(f"Unsupported sample rate: {self.settings.audio_sample_rate}")
    ...
    async def start(self) -> None:
        ...
        with with_correlation_id(self.correlation_id):
            try:
                ...
                self.logger.info("SIP AI Agent started successfully",
                               sip_domain=self.settings.sip_domain,
    ...
    async def _register_sip(self) -> None:
        ...
        while retry_count < max_retries and self.is_running:
            try:
                self.logger.info("Attempting SIP registration",
                ...
                retry_count += 1
                self.logger.warning("SIP registration failed",
                                  attempt=retry_count,
                                  max_retries=max_retries,
                                  error=str(e))
                
                if retry_count < max_retries:
                    backoff_time = self.settings.sip_registration_retry_backoff ** retry_count
                    self.logger.info("Retrying SIP registration",
                                   retry_in_seconds=backoff_time)
        ...
        try:
            while self.is_running:
                ...
                if self.sip_client and not self.sip_client.is_registered():
                    self.logger.warning("SIP registration lost, attempting to re-register")
                    monitor.update_registration(False)
                    try:
                        await self._register_sip()
                    except Exception as e:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            # Final health report
            report = await self.health_monitor.run_health_checks()
            self.logger.info("Final health report",
                           overall_status=report.overall_status.value,
