...
class TestOpenAIAgent:
    ...
    @pytest.mark.asyncio
    async def test_realtime_audio_sending(self, mock_agent, mock_call, sample_audio_frame):
    ...
    @pytest.mark.asyncio
    async def test_legacy_audio_sending(self, mock_agent, mock_call, sample_audio_frame):
    ...
    async def test_websocket_connection_handling(self, mock_agent):
        ...
        with patch('openai_agent.websockets.connect') as mock_connect:
            ...
            with patch.object(mock_agent, '_send_audio_realtime') as mock_send, \
...
class TestOpenAIAgentIntegration:
    ...
    async def test_full_call_flow_realtime(self, test_settings, sample_audio_frame):
        ...
        with patch('openai_agent.get_settings') as mock_get_settings:
            ...
            mock_call.audio_callback = Mock()
            mock_call.audio_callback.is_active = True
            mock_call.audio_callback.get_audio_frame = AsyncMock(return_value=sample_audio_frame)
            mock_call.playback_audio = Mock()
    ...
    @pytest.mark.asyncio
    async def test_token_usage_tracking(self, mock_agent, mock_call, sample_audio_frame):
