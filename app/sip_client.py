...
from logging_config import get_logger, log_sip_event, with_correlation_id, generate_correlation_id
...
class EnhancedAudioCallback(pj.AudioMedia):
    ...
    def __init__(self, call, correlation_id: str):
        ...
        self.audio_queue = asyncio.Queue(maxsize=get_settings().audio_backpressure_threshold)
        self.is_active = True
        self.frame_count = 0
    ...
    def onFrameRequested(self, frame):
        ...
        try:
            if frame.type == pj.PJMEDIA_FRAME_TYPE_AUDIO and frame.buf:
                ...
                if self.audio_queue.qsize() >= get_settings().audio_backpressure_threshold:
                    metrics.record_audio_dropout(self.correlation_id, "backpressure")
                    self.dropout_count += 1
                    logger.warning("Audio backpressure detected", 
                ...
                try:
                    ...
                    self.frame_count += 1
                    metrics.update_audio_queue_size(self.correlation_id, self.audio_queue.qsize())
                except asyncio.QueueFull:
                    metrics.record_audio_dropout(self.correlation_id, "queue_full")
                    self.dropout_count += 1
                    
        except Exception as e:
            logger.error("Error in audio frame processing", 
    ...
    def stop(self):
        ...
        logger.info("Audio callback stopped", 
                   correlation_id=self.correlation_id,
    ...
    async def get_audio_frame(self) -> Optional[bytes]:
        ...
        try:
            ...
            logger.error("Error getting audio frame", 
                        correlation_id=self.correlation_id, error=str(e))
            return None
...
class EnhancedCall(pj.Call):
    ...
    def onCallState(self, prm):
        ...
        with with_correlation_id(self.correlation_id):
            log_sip_event("call_state_change", 
    ...
    def _start_openai_agent(self):
        ...
        try:
            ...
            self.openai_thread = threading.Thread(
                target=asyncio.run,
                args=(agent.start(self),),
                daemon=True
            )
            self.openai_thread.start()
            logger.info("OpenAI agent started", 
                       call_id=self.call_id,
                       correlation_id=self.correlation_id)
        except Exception as e:
    ...
    def playback_audio(self, audio_data: bytes):
        ...
        try:
            ...
            logger.debug("Audio played back", 
                        call_id=self.call_id,
                        correlation_id=self.correlation_id,
                        size=len(audio_data))
            
        except Exception as e:
            logger.error("Error playing back audio", 
                        call_id=self.call_id,
                        correlation_id=self.correlation_id,
                        error=str(e))
...
class EnhancedAccount(pj.Account):
    ...
    def onRegState(self, prm):
        ...
        log_sip_event("registration_state_change",
        ...
        if reg_status == 200:
            ...
            if self.registration_start_time > 0:
                ...
                metrics.record_sip_registration_attempt(
                    self.settings.sip_domain,
                    self.settings.sip_user,
                    True,
                    duration
                )
    ...
    def _schedule_reconnect(self):
        ...
        backoff_time = self.settings.sip_registration_retry_backoff ** self.registration_attempts
        
        logger.info("Scheduling SIP reconnection",
                   domain=self.settings.sip_domain,
                   user=self.settings.sip_user,
                   attempt=self.registration_attempts,
                   backoff_seconds=backoff_time)
        
        self.reconnect_task = asyncio.create_task(self._reconnect_after_delay(backoff_time))
    ...
    async def _attempt_registration(self):
        ...
        try:
            ...
            cred = pj.AuthCredInfo("digest", "*", self.settings.sip_user, 0, self.settings.sip_pass)
            acc_cfg.sipConfig.authCreds.append(cred)
            
            # Configure codecs
            if hasattr(acc_cfg, 'mediaConfig') and hasattr(acc_cfg.mediaConfig, 'codecConfig'):
            ...
            logger.info("SIP registration attempt initiated",
                       domain=self.settings.sip_domain,
                       user=self.settings.sip_user,
                       attempt=self.registration_attempts)
            
        except Exception as e:
            logger.error("Error during SIP registration attempt",
                        domain=self.settings.sip_domain,
                        user=self.settings.sip_user,
                        error=str(e))
            self._schedule_reconnect()
    ...
    def onIncomingCall(self, prm):
        ...
        try:
            ...
            logger.error("Error answering incoming call",
                        call_id=prm.callId,
                        error=str(e))
    ...
    def initialize(self) -> None:
        ...
        try:
            ...
            if self.settings.sip_transport.upper() == "TCP":
                ...
                transport_cfg.tlsConfig.certFile = getattr(self.settings, 'sip_cert_file', '')
                transport_cfg.tlsConfig.privKeyFile = getattr(self.settings, 'sip_key_file', '')
